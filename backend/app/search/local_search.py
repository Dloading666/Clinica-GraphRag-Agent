...
class LocalSearch:
    ...
    async def search(self, query: str, db: AsyncSession) -> Dict:
        try:
            ...
            entity_names = await self._find_entities_by_text(
                query, db, top_k=settings.search.local_top_entities
            )
            chunks = await self._search_chunks_by_text(
                query, db, top_k=settings.search.local_top_chunks
            )
    ...
    async def _find_entities_by_text(
        ...
        patterns = [clean_query, *_tokenize(clean_query)]
        where_clauses = []
        ...
        ranked = sorted(
            rows,
            key=lambda row: self._score_entity(row.name, row.description, clean_query),
            reverse=True,
        )
        ...
        seen = set()
        for row in ranked:
            score = self._score_entity(row.name, row.description, clean_query)
    ...
    def _score_entity(self, name: str, description: Optional[str], query: str) -> float:
